...
You do **not** always have to insert a reference's target before the node that points at it. When a `target` (or a `SET`/`REFERENCE` `targetRefOrName`) is a plain **name** rather than an `r:`/`i:` persistent ref, the blueprint reader stores it as a **dynamic reference** that MPS resolves later by that name. This lets you author one logical unit — e.g. a `Root_MappingRule` together with the template it points at — without strictly ordering the inserts, as long as the name is unambiguous in the resolution scope.
