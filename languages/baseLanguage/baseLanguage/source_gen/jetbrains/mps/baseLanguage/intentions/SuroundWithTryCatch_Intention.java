...
public final class SuroundWithTryCatch_Intention extends AbstractIntentionDescriptor implements IntentionFactory {
  ...
  /*package*/ final class IntentionImplementation extends AbstractIntentionExecutable {
    ...
    public void execute(final SNode node, final EditorContext editorContext) {
      final SNode tryCatchStatement = SNodeFactoryOperations.createNewNode(CONCEPTS.TryCatchStatement$XR, null);
      List<SNode> selectedNodes = editorContext.getSelectedNodes();
      SNodeOperations.insertNextSiblingChild(node, tryCatchStatement);
      for (SNode selectedNode : ListSequence.fromList(selectedNodes)) {
        ListSequence.fromList(SLinkOperations.getChildren(SLinkOperations.getTarget(tryCatchStatement, LINKS.body$pDF2), LINKS.statement$53DE)).addElement(SNodeOperations.getNodeAncestor(selectedNode, CONCEPTS.Statement$P6, true, false));
      }
      Iterable<SNode> caughtExceptions = ListSequence.fromList(SNodeOperations.getNodeAncestors(tryCatchStatement, CONCEPTS.ITryCatchStatement$pH, false)).translate(new ITranslator2<SNode, SNode>() {
        public Iterable<SNode> translate(SNode it) {
          return SLinkOperations.collect(SLinkOperations.collect(ITryCatchStatement__BehaviorDescriptor.getCatchClauses_id3eptmOG0XgA.invoke(it), LINKS.throwable$$5MH), LINKS.type$a1UY);
        }
      ...
      Set<SNode> uncaughtThrowables = StatementList__BehaviorDescriptor.uncaughtThrowables_id2SVUfbZ9Qq1.invoke(SLinkOperations.getTarget(tryCatchStatement, LINKS.body$pDF2), ((boolean) false));
      if (SetSequence.fromSet(uncaughtThrowables).isNotEmpty()) {
        ListSequence.fromList(SLinkOperations.getChildren(tryCatchStatement, LINKS.catchClause$dMnP)).clear();
        SetSequence.fromSet(uncaughtThrowables).visitAll(new IVisitor<SNode>() {
          public void visit(SNode it) {
            ...
            SNode clause = SNodeFactoryOperations.createNewNode(CONCEPTS.CatchClause$Ig, null);
            SLinkOperations.setTarget(clause, LINKS.throwable$$5MH, SNodeFactoryOperations.createNewNode(CONCEPTS.LocalVariableDeclaration$41, null));
            SLinkOperations.setTarget(SLinkOperations.getTarget(clause, LINKS.throwable$$5MH), LINKS.type$a1UY, type);
            SPropertyOperations.assign(SLinkOperations.getTarget(clause, LINKS.throwable$$5MH), PROPS.name$MnvL, "e");
            ListSequence.fromList(SLinkOperations.getChildren(tryCatchStatement, LINKS.catchClause$dMnP)).addElement(clause);
          }
        });
        if (ListSequence.fromList(SLinkOperations.getChildren(tryCatchStatement, LINKS.catchClause$dMnP)).isEmpty()) {
          SNodeFactoryOperations.addNewChild(tryCatchStatement, LINKS.catchClause$dMnP, CONCEPTS.CatchClause$Ig);
        }
      }
      editorContext.select(SLinkOperations.getTarget(SLinkOperations.getTarget(ListSequence.fromList(SLinkOperations.getChildren(tryCatchStatement, LINKS.catchClause$dMnP)).first(), LINKS.throwable$$5MH), LINKS.type$a1UY));
    }
    @Override
