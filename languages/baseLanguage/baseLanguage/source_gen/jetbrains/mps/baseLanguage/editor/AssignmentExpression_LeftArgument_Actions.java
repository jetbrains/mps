  ...
  /*package*/ static AbstractCellAction createAction_DELETE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (DeletionApproverUtil.approve(editorContext, SLinkOperations.getTarget(node, LINKS.lValue$splI))) {
          return;
        }
        if (SConceptOperations.isExactly(SNodeOperations.asSConcept(SNodeOperations.getConcept(SLinkOperations.getTarget(node, LINKS.lValue$splI))), SNodeOperations.asSConcept(CONCEPTS.Expression$mB))) {
          SNodeOperations.replaceWithAnother(node, SLinkOperations.getTarget(node, LINKS.rValue$spNK));
        } else {
          SLinkOperations.setTarget(node, LINKS.lValue$splI, SConceptOperations.createNewNode(MetaAdapterFactory.getConcept(0xf3061a5392264cc5L, 0xa443f952ceaf5816L, 0xf8c37f506fL, "jetbrains.mps.baseLanguage.structure.Expression")));
        }
      }
