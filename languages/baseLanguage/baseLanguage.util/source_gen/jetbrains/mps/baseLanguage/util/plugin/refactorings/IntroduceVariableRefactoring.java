...
public abstract class IntroduceVariableRefactoring {
  ...
  public String init(SNode node, JComponent editorComponent) {
    ...
    if (this.isVoidType(expressionType)) {
      ...
      if (SNodeOperations.isInstanceOf(node, CONCEPTS.Expression$mB)) {
        expectedVariableName = Expression__BehaviorDescriptor.getVariableExpectedName_idhEwJgm_.invoke(SNodeOperations.cast(node, CONCEPTS.Expression$mB));
      }
      if (expectedVariableName != null) {
        ListSequence.fromList(expectedNames).addElement(NameUtil.decapitalize(expectedVariableName));
      }
      List<String> variableSuffixes = Type__BehaviorDescriptor.getVariableSuffixes_idhEwIzNo.invoke(myExpressionType);
  ...
  protected String initForVariable(SNode var, SNode initializer, JComponent editorComponent) {
    ...
    if (this.isVoidType(expressionType)) {
      ...
      this.myExpressionType = SNodeOperations.cast(expressionType, CONCEPTS.Type$bu);
      String variableExpectedName = Expression__BehaviorDescriptor.getVariableExpectedName_idhEwJgm_.invoke(Type__BehaviorDescriptor.createDefaultTypeExpression_id2UvJdVpqUA4.invoke(this.myExpressionType));
      List<String> expectedNames = ListSequence.fromList(new ArrayList<String>());
  ...
  private SNode getExpressionType(SNode node) {
    ...
    if (SNodeOperations.isInstanceOf(expressionType, CONCEPTS.IInternalType$eR)) {
      expressionType = IInternalType__BehaviorDescriptor.getPublicType_idhEwIXGa.invoke(SNodeOperations.cast(expressionType, CONCEPTS.IInternalType$eR));
    }
