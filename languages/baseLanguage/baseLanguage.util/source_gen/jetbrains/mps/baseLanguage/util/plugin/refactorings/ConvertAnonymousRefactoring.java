...
public class ConvertAnonymousRefactoring {
  ...
  public void doRefactor() {
    ...
    if ((creator != null)) {
      SNodeOperations.replaceWithAnother(creator, makeInnerConstructorInvocation(Sequence.fromIterable(ClassConcept__BehaviorDescriptor.constructors_id4_LVZ3pCvsd.invoke(makeInnerClass())).first()));
    }
  }
  ...
  private void makeInnerConstructor(SNode innerClass) {
    if (Sequence.fromIterable(ClassConcept__BehaviorDescriptor.constructors_id4_LVZ3pCvsd.invoke(innerClass)).isEmpty()) {
      SNode ctor = SModelOperations.createNewNode(SNodeOperations.getModel(innerClass), null, CONCEPTS.ConstructorDeclaration$yG);
      Classifier__BehaviorDescriptor.insertInBestPlace_id7exmRT6rEUA.invoke(innerClass, ctor);
    ...
    SNode innerConstructor = Sequence.fromIterable(ClassConcept__BehaviorDescriptor.constructors_id4_LVZ3pCvsd.invoke(innerClass)).first();
  ...
  private void makeConstructorBody(SNode constructorDeclaration) {
    ...
    for (SNode fieldDeclaration : SetSequence.fromSet(MapSequence.fromMap(this.myInnerConstructorParameters).keySet())) {
      ListSequence.fromList(SLinkOperations.getChildren(SLinkOperations.getTarget(constructorDeclaration, LINKS.body$5xQk), LINKS.statement$53DE)).addElement(_quotation_createNode_qy1soj_a0a0a1a61(SNodeOperations.getParent(constructorDeclaration), fieldDeclaration, VariableDeclaration__BehaviorDescriptor.createReference_idhEwJfME.invoke(MapSequence.fromMap(this.myInnerConstructorParameters).get(fieldDeclaration))));
    }
  }
  ...
  private void addFieldsToInnerClass(SNode innerClass) {
    for (SNode field : Sequence.fromIterable(MapSequence.fromMap(this.myInnerFields).values())) {
      Classifier__BehaviorDescriptor.insertInBestPlace_id7exmRT6rEUA.invoke(innerClass, field);
    }
  ...
  private static boolean check_qy1soj_a0a52(SNode checkedDotOperand) {
    if (null != checkedDotOperand) {
      return (boolean) ClassifierMember__BehaviorDescriptor.isStaticClassifierMember_id7MS72Gc8avw.invoke(checkedDotOperand);
    }
    return false;
  }
