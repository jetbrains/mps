...
public class check_ImplementationCode_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode n, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if ((SLinkOperations.getTarget(SLinkOperations.getTarget(n, LINKS.descriptor$6cHY), LINKS.implModel$pBbn) == null)) {
      {
    ...
    } else if (!(SNodeOperations.getModel(n).getReference().equals(ModelIdentity__BehaviorDescriptor.toModelReference_id1Bs_61$mvvu.invoke(SLinkOperations.getTarget(SLinkOperations.getTarget(n, LINKS.descriptor$6cHY), LINKS.implModel$pBbn))))) {
      {
        final MessageTarget errorTarget = new NodeMessageTarget();
