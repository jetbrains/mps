...
public class GenericTupleTupleArray_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  public GenericTupleTupleArray_NonTypesystemRule() {
  }
  public void applyRule(final SNode arrayCreator, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    // extracted from bl.genericArrayCreatorsAreIllegal
    // alghough not sure it's possible to use NamedTupleType in new Classifier<X,Y>[]; in my attempts, it's always ClassifierType, not NamedTupleType
      ...
      if (ListSequence.fromList(SLinkOperations.getChildren(SNodeOperations.cast(componentType, CONCEPTS.NamedTupleType$DW), LINKS.parameter$oqG$)).any((it) -> !(SNodeOperations.isInstanceOf(it, CONCEPTS.WildCardType$uV)))) {
        {
          ...
          IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(componentType, "illegal generic array creation", "r:e119dbbd-3529-4067-8bad-6b9edd79d0b6(jetbrains.mps.baseLanguage.tuples.typesystem)", "2675087449870928909", null, errorTarget);
        }
      }
    }
  }
