...
public class AttributeDesignTimeOperations {
  public static Iterable<SNode> getApplicableAttributes(SNode accessNode, final SNodeReference attributeType) {
    ...
    return Sequence.fromIterable(SNodeOperations.ofConcept(conceptsScope.getAvailableElements(null), CONCEPTS.ConceptDeclaration$gH)).where((it) -> {
      ...
      return isNotEmptyString(SPropertyOperations.getString(new IAttributeDescriptor.NodeAttribute(CONCEPTS.AttributeInfo$hg).get(it), PROPS.role$vUyK)) && Sequence.fromIterable(getApplicableConcepts(it)).any(new _FunctionTypes._return_P1_E0<Boolean, SNode>() {
        public Boolean invoke(SNode it) {
  ...
  public static boolean isAttributeDeclaration(SNode conceptDeclaration) {
    return SetSequence.fromSet(getSuperConcepts(conceptDeclaration)).any((it) -> SNodeOperations.is(it, new SNodePointer("r:00000000-0000-4000-0000-011c89590288(jetbrains.mps.lang.core.structure)", "5169995583184591161")));
  }
  public static boolean isMultipleAttribute(SNode attributeDeclaration) {
    if (!(isAttributeDeclaration(attributeDeclaration))) {
      return false;
    }
    return SPropertyOperations.getBoolean(SLinkOperations.getTarget(new IAttributeDescriptor.NodeAttribute(CONCEPTS.AttributeInfo$hg).get(SetSequence.fromSet(getSuperConcepts(attributeDeclaration)).findFirst((it) -> (SLinkOperations.getTarget(new IAttributeDescriptor.NodeAttribute(CONCEPTS.AttributeInfo$hg).get(it), LINKS.multiple$eysG) != null))), LINKS.multiple$eysG), PROPS.value$JccR);
  }
  @Nullable
  public static String getAttributeRole(SNode attributeDeclaration) {
    ...
    return SPropertyOperations.getString(new IAttributeDescriptor.NodeAttribute(CONCEPTS.AttributeInfo$hg).get(SetSequence.fromSet(getSuperConcepts(attributeDeclaration)).findFirst((it) -> isNotEmptyString(SPropertyOperations.getString(new IAttributeDescriptor.NodeAttribute(CONCEPTS.AttributeInfo$hg).get(it), PROPS.role$vUyK)))), PROPS.role$vUyK);
  }
