...
public class DeleteAttribute {
    ...
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (!(DeletionApproverUtil.approve(editorContext, node))) {
          SNode candidate = SNodeOperations.getNextSibling(node);
          if (candidate != null) {
            SelectionUtil.selectCell(editorContext, candidate, SelectionManager.FIRST_CELL);
          } else {
            candidate = SNodeOperations.getPrevSibling(node);
            if (candidate != null) {
              SelectionUtil.selectCell(editorContext, candidate, SelectionManager.LAST_CELL);
            } else {
              SelectionUtil.selectLabelCellAnSetCaret(editorContext, SNodeOperations.getParent(node), SelectionManager.FIRST_EDITABLE_CELL, -1);
            }
          }
  ...
  /*package*/ static AbstractCellAction createAction_BACKSPACE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        if (!(DeletionApproverUtil.approve(editorContext, node))) {
          SNode candidate = SNodeOperations.getPrevSibling(node);
          if (candidate != null) {
            SelectionUtil.selectCell(editorContext, candidate, SelectionManager.LAST_CELL);
          } else {
            SelectionUtil.selectLabelCellAnSetCaret(editorContext, SNodeOperations.getParent(node), SelectionManager.FIRST_EDITABLE_CELL, -1);
