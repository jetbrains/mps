 ...
 * @since 2021.1
 */
final class ClassFileImpl implements ClassFile {
  private final String myQualifiedName;
  private final byte[] myBytes;
  private final boolean myHasErrors;

  /*package*/ ClassFileImpl(String qualifiedName, byte[] bytes, boolean hasErrors) {
  ...
  /*package*/ static ClassFile[] from(CompilationResult result) {
    ...
    for (int i = 0; i < ecjFiles.length; i++) {
      ...
      rv[i] = new ClassFileImpl(fqn, ecjFiles[i].getBytes(), gotErrors);
    }
    return rv;
  }
