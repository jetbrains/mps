package jetbrains.mps.baseLanguage.javastub.asm;

/*Generated by MPS */

import jetbrains.mps.annotations.GeneratedClass;
import java.util.Map;
import java.util.HashMap;

@GeneratedClass(nodeId = "7241381882860002170", model = "r:eafb5d8e-2952-4826-b4ad-be2b9011f598(jetbrains.mps.baseLanguage.javastub.asm)")
public class ASMClassType extends ASMType {
  public static final ASMClassType OBJECT = new ASMClassType("java.lang.Object");
  private final String myName;

  public ASMClassType(String name) {
    myName = name;
  }
  public String getName() {
    return myName;
  }
  @Override
  public String toString() {
    return "(" + myName + ")";
  }

  @Override
  public int hashCode() {
    return myName.hashCode();
  }
  @Override
  public boolean equals(Object obj) {
    if (this == obj) {
      return true;
    }
    return obj != null && obj.getClass() == getClass() && myName.equals(((ASMClassType) obj).myName);
  }

  public static final class Factory {
    private final Map<String, ASMClassType> myCache = new HashMap<>();
    {
      myCache.put(OBJECT.myName.replace('.', '/'), OBJECT);
      myCache.put(String.class.getName().replace('.', '/'), new ASMClassType(String.class.getName()));
    }

    public void record(String classQualifiedName) {
      myCache.put(classQualifiedName.replace('.', '/'), new ASMClassType(classQualifiedName));
    }

    /*package*/ ASMClassType fromBinaryName(String internalClassNamePresentation) {
      ASMClassType c = myCache.get(internalClassNamePresentation);
      if (c != null) {
        return c;
      }
      return new ASMClassType(internalClassNamePresentation.replace('/', '.'));
    }

    /*package*/ ASMClassType fromQualifiedName(String classNamePresentation) {
      return new ASMClassType(classNamePresentation);
    }
  }
}
