...
public class ModuleDependenciesTest extends ModuleMpsTest {
  ...
  public WriteAction wa = new WriteAction(() -> {
    // Environment instance is not available the moment @Rule value is consulted, hence Supplier.
  ...
  private void testDependency(SModule source, SModule target, boolean showRuntime, int numPaths) {
    ...
    Assert.assertEquals(numPaths, paths.size());
  }
  ...
  public void testRuntimeDependencies() {
    ...
    */
    ...
    // runtime dependencies are off
    testDependency(solutions[0], solutions[1], false, 0);
    // runtime dependencies are on
    testDependency(solutions[0], solutions[1], true, 1);
    // runtime dependencies can pass through extended language
    testDependency(solutions[0], solutions[2], true, 1);
    // runtime dependencies can not pass through used language
  ...
  public void testLanguageAndDevkitRuntimeDependencies() {
    ...
    */
    ...
    // runtime dependencies can not pass through used language
    testDependency(solutions[0], solutions[3], true, 0);
    // runtime dependencies can pass through devkits:
    // (used) Devkit exports the solution[5]
    testDependency(solutions[0], solutions[5], true, 1);
    // runtime dependencies can pass through devkits
    testDependency(solutions[0], solutions[6], true, 1);
    // runtime dependencies can pass through devkits and language
    testDependency(solutions[0], solutions[7], true, 1);
    // devkit language is not dependency
  ...
  public void testUsedLanguages() {
    ...
    */
    ...
    // simple
    testUsedLanguage(l0, languages[1], 1);
    // extended language is usedLanguage
    testUsedLanguage(l0, languages[2], 1);
    testUsedLanguage(l0, languages[3], 0);
    // one as exported from devKit, another as usedLanguage (resolved devkit)
    testUsedLanguage(l1, languages[4], 2);
    // extended lang + devKit
    testUsedLanguage(l0, languages[4], 0);
    // One as exported from extended DevKit, another as usedLanguage (resolved extended devkit)
    testUsedLanguage(l1, languages[5], 2);
    // if one uses a language, it takes it as a whole, with all extended languages
    testUsedLanguage(l1, languages[6], 1);
  }
  ...
  public void testExtendedLanguageAsDependencies() {
    ...
    // extends is dependency
    testDependency(languages[0], languages[1], false, 1);
    // extends is re-exported dependency
    testDependency(solutions[1], languages[1], false, 1);
    // extends is re-exported dependency, cannot go through nonexported dependency
    testDependency(solutions[0], languages[1], false, 0);
    // runtime dependencies can go through other dependencies
    testDependency(solutions[0], languages[1], true, 1);
  }
