...
public class NullValues_Test extends Util_Test {
  ...
  public void test_nullValues() throws Exception {
    if (Sequence.IGNORE_NULL_VALUES) {
      ...
      this.assertIterableEquals(this.expectEmpty(), Sequence.fromIterable(input).select((it) -> (Integer) null));
      this.assertIterableEquals(this.expectEmpty(), Sequence.fromIterable(input).translate((it) -> {
...
__switch__:
      ...
      Iterable<Integer> input = this.input5();
      Iterable<Integer> seqOfNulls = Sequence.fromIterable(input).select((it) -> (Integer) null);
      Assert.assertSame(Sequence.fromIterable(input).count(), Sequence.fromIterable(seqOfNulls).count());
      Assert.assertTrue(Sequence.fromIterable(seqOfNulls).all((it) -> it == null));
      Iterable<Integer> seqOfNulls2 = Sequence.fromIterable(input).translate((it) -> {
        return (Iterable<Integer>) () -> {
          return new YieldingIterator<Integer>() {
            private int __CP__ = 0;
            protected boolean moveToNext() {
...
__switch__:
      ...
      Assert.assertFalse(Sequence.fromIterable(seqOfNulls2).any((it) -> it != null));
    }
  }
  @Test
  public void test_nullElements() throws Exception {
    if (Sequence.IGNORE_NULL_VALUES) {
      ...
      Assert.assertSame(3, Sequence.fromIterable(Sequence.fromClosure(() -> {
        return (Iterable<Integer>) () -> {
          return new YieldingIterator<Integer>() {
            private int __CP__ = 0;
            protected boolean moveToNext() {
...
__switch__:
  ...
  public void test_nullsViaBackdoor() throws Exception {
    if (Sequence.IGNORE_NULL_VALUES) {
      ...
      List<String> dlist = ListSequence.fromList(listWithNulls).distinct().toList();
