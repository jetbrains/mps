  ...
  protected JComponent createCenterPanel() {
    ...
    if (ExperimentalUI.isNewUI()) {
      ...
      myHeader.getPanel().setBorder(JBUI.Borders.compound(JBUI.Borders.customLineBottom(JBUI.CurrentTheme.CustomFrameDecorations.separatorForeground()),
                                                     new JBEmptyBorder(0, 20, 0, 15)));
      myHeader.getPanel().setBackground(JBUI.CurrentTheme.Popup.headerBackground(true));
      dialogPanel.setBackground(background);
      mySearchEntry.setOpaque(false);
      Border compound = Borders.compound(new JBEmptyBorder(JBUI.insets(0, 12)),
                                         Borders.compound(Borders.customLine(CustomFrameDecorations.separatorForeground(), 0, 0,
                                                                             1, 0),
                                                          new JBEmptyBorder(JBUI.insets(6, 2))));
      mySearchEntry.setBorder(compound);
      myResultsPreviewTable.setBackground(background);

      var textFieldBorderInsets = JBUI.insets(0, 12);
