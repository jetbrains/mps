...
/*package*/ class ClassifierCacher {
  private final IdDataConsumer myResult = new IdDataConsumer();

  public ClassifierCacher() {
  }
  public Map<IdIndexEntry, Integer> getResult() {
    // for index queries, like FileBasedIndex.getContainingFiles(), we need IdIndexEntry, while to create, it's advised to use IdDataConsumer to build
    // a custom Map implementation. Despite IdDataConsumer.addOccurence doesn't use the same code as new IdIndexEntry()->getWordHash(), IDEA team promises
    // we should not worry and trust them to work the same.
    // Note, caseSensitive == true argument for IdIndexEntry is not of importance as IdDataConsumer records both case-sensitive and case-insensitive hash values, 
    // we could use both true and false to look them up.
    return myResult.getResult();
  }

  /*package*/ static IdIndexEntry forConcept(SAbstractConcept concept) {
    return new IdIndexEntry(_forConcept(concept), true);
  }
  private static String _forConcept(SAbstractConcept concept) {
  ...
  /*package*/ static IdIndexEntry forNode(SNodeId id) {
    return new IdIndexEntry(_forNode(id), true);
  }
  private static String _forNode(SNodeId id) {
    return id.toString();
  }
  ...
  /*package*/ static IdIndexEntry forModel(SModelId id) {
    return new IdIndexEntry(_forModel(id), true);
  }
  private static String _forModel(SModelId id) {
    return id.toString();
  }

  private void instance(SAbstractConcept concept) {
    String s = _forConcept(concept);
    // occurrenceMask is something like UsageSearchContext. Can't be 0, but any other value will do, as MPS doesn't use it
    myResult.addOccurrence(s, 0, s.length(), 0xff);
  }
  private void ref(SNodeId id) {
    ...
    String s = _forNode(id);
    myResult.addOccurrence(s, 0, s.length(), 0xff);
  }
  private void modelRef(String packageName) {
    String s = _forModel(new JavaPackageNameStub(packageName).asModelId());
    myResult.addOccurrence(s, 0, s.length(), 0xff);
  }
