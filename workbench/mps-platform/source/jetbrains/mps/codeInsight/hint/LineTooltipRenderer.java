...
/**
 * Copied from {@link com.intellij.codeInsight.hint.LineTooltipRenderer}.
 */
public class LineTooltipRenderer extends ComparableObject.Impl implements TooltipRenderer {
  ...
  public LineTooltipRenderer(@Nullable final String text, final int width, Object[] comparable) {
    super(comparable);
  ...
  public LightweightHint show(@NotNull EditorComponent editorComponent,
                              @NotNull final Point p,
                              final boolean alignToRight,
                              @NotNull final TooltipGroup group,
                              @NotNull final HintHint hintHint) {
    LightweightHint hint = createHint(editorComponent, p, alignToRight, group, hintHint, true, true,
                                      null);
    if (hint != null) {
      HintManagerImpl.getInstanceImpl().showEditorHint(hint, editorComponent, p, HintManager.HIDE_BY_ANY_KEY |
  ...
  public LightweightHint createHint(EditorComponent editorComponent,
                                    @NotNull final Point p,
                                    final boolean alignToRight,
                                    @NotNull final TooltipGroup group,
                                    @NotNull final HintHint hintHint,
    ...
    String dressedText = dressDescription(tooltipPreText, myCurrentWidth > 0);

    final boolean expanded = myCurrentWidth > 0 && !dressedText.equals(tooltipPreText);

    if (!editorComponent.getComponent().isShowing()) {
      return null;
    }
    final JLayeredPane layeredPane = editorComponent.getComponent().getRootPane().getLayeredPane();
...
//    }
    ...
    final LightweightHint hint = new LightweightHint(grid) {
      ...
      public void hide() {
        ...
        for (AnAction action : actions) {
          action.unregisterCustomShortcutSet(editorComponent.getContentComponent());
        }
      }
    ...
    TooltipReloader reloader = tooltipReloader == null
                               ? toExpand -> reloadFor(hint, editorComponent, p, editorPane, alignToRight, group,
                                                       hintHint, toExpand)
                               : tooltipReloader;
    ...
    reloadAction.registerCustomShortcutSet(KeymapUtil.getActiveKeymapShortcuts(IdeActions.ACTION_SHOW_ERROR_DESCRIPTION),
                                           editorComponent.getContentComponent());
    ...
    editorPane.addHyperlinkListener(e -> {
      ...
      if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
        ...
        if (description != null &&
            handle(description, editorComponent)) {
          hint.hide();
          return;
        }
        Project ideaProject = ProjectHelper.toIdeaProject(editorComponent.getProject());
        TooltipActionsLogger.logShowDescription(ideaProject, TooltipActionsLogger.Source.MoreLink, e.getInputEvent(), null);
        reloader.reload(!expanded);
      }
    });

    fillPanel(grid, hint, hintHint, actions, reloader, highlightActions);
  ...
  private void reloadFor(@NotNull LightweightHint hint,
    ...
    TooltipController.getInstance().showTooltip(editorComponent, point,
                                                createRenderer(myText, expand ? pane.getWidth() : 0), alignToRight, group,
                                                hintHint);
  }

  protected void fillPanel(@NotNull JPanel component,
  ...
  private static boolean handle(@NotNull final String ref, EditorComponent editorComponent) {
    return TooltipLinkHandlerEP.handleLink(ref, editorComponent);
  }
