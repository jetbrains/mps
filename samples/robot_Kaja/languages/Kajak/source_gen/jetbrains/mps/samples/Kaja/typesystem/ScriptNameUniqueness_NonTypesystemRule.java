...
public class ScriptNameUniqueness_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode script, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    Iterable<String> scriptNames = ListSequence.fromList(SModelOperations.roots(SNodeOperations.getModel(script), CONCEPTS.Script$FS)).select((it) -> SPropertyOperations.getString(it, PROPS.name$MnvL));
    Iterable<String> libraryNames = ListSequence.fromList(SModelOperations.roots(SNodeOperations.getModel(script), CONCEPTS.Library$oJ)).select((it) -> SPropertyOperations.getString(it, PROPS.name$MnvL));
    ...
    if (ListSequence.fromList(allNames).where((it) -> it != null && it.equals(SPropertyOperations.getString(script, PROPS.name$MnvL))).count() > 1) {
      {
