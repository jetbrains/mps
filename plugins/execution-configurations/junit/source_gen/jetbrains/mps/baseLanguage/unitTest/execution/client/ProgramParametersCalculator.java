...
/*package*/ class ProgramParametersCalculator {
  ...
  private String calcParamsWithMpsPlatformToStart() throws ExecutionException {
    ...
    // XXXMaywanttopassvalueofidea.additional.classpathsystempropertyfurthertonewIdeaApplicationinstancetoensurepluginsthatare
    // loadedfromsourcescouldgetloadedinthenewapplicationaswell.
    for (ITestNodeWrapper test : ListSequence.fromList(myTestsToRun.getTests())) {
      args.addTest(test);
    }
    try {
  ...
  private void addModulesAndDepsToStartupArgs(@Mutable ScriptData startupArgs) {
    ...
    // I'vegotsetofreferencetomodulesIneedtopresentinanewMPSinstance
    // andnowhavetoguesstheirlocationstopasstothenewinstance.
    // XXXhere,weexploittheassumptionmoduledescriptorfileresidesunderamoduleroot
    myRepo.getModelAccess().runReadAction(new Runnable() {
      public void run() {
        ...
        // XXXdon'tweneedtoaddrespectivegeneratormodulejarslikewedoinMpsTestsSuiteandGenerateTasksotha
        // environmentstartedfromWithPlatformTestExecutorloadsallmodulesproperly?
        // OTOH,languageandgeneratormodulesfromtheMPSplatformarelikelytogetloadedregardlessofthesetting,
        // whilelanguagesandgeneratorsfromtheactiveprojectarenon-deployedanyway
        for (SModule m : CollectionSequence.fromCollection(execClosure)) {
          ...
          // XXXinfact,fornon-deployedmodulethiswouldendupwithamodulesourcedir,whichisnot'file'perse,butaslongasthere's
          // nodistinctioninprocessingrd.filesandrd.folders(ModulesMinerdoesn'tcare),Idon'tbotherhereeither.
  ...
  private Set<SModule> collectExecuteCP(Iterable<SModule> modules) {
    ...
    // usedlangforruntimesweneedsincewearenotcapableoflocatingtheruntimeclasses
    // whenwearerunningjavaclassforexample
  ...
  private void addMacrosToStartupArgs(@Mutable ScriptData startupArgs) {
    // FIXMEShalluseproperComponentHost.findComponenttoaccessPathMacrosinstance
    PathMacros pathMacros = PathMacros.getInstance();
    // XXXnotsurewhyweiterateoverusernamesonly(notgetNames()),it'sthewayitusedtobeinLanguageTestWrapper/AbstractTestWrapperforalongtime
    for (String key : pathMacros.getUserNames()) {
      String value = pathMacros.getValue(key);
      if (value != null) {
        startupArgs.addMacro(key, value);
      } else {
        // XXXEnvironmentBaseisnotquitefriendlytonullmacrovalues.Ican'tdecidewhetherit'sbettertorelaxthisrestriction(whocareswhatmacrovalueis
        // exceptitsconsumer?),toreportawarninghereortoletEnvironmentBasedothat.
