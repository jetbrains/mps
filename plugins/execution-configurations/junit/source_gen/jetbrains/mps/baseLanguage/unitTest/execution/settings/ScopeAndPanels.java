...
public final class ScopeAndPanels {
  ...
  public ScopeAndPanels(@NotNull com.intellij.openapi.project.Project project) {
    ...
    MapSequence.fromMap(myScopePanels).put(JUnitRunTypes.PROJECT, new PanelPerScope(projectPanel));
    MapSequence.fromMap(myScopePanels).put(JUnitRunTypes.MODULE, new PanelPerScope(modulePanel));
    MapSequence.fromMap(myScopePanels).put(JUnitRunTypes.MODEL, new PanelPerScope(modelPanel));
    MapSequence.fromMap(myScopePanels).put(JUnitRunTypes.NODE, new PanelPerScope(myClassesChooser));
    MapSequence.fromMap(myScopePanels).put(JUnitRunTypes.METHOD, new PanelPerScope(myMethodsChooser));
  ...
  public List<InProcessJBCheckBox> getInProcessCheckBoxes() {
    ...
    for (PanelPerScope panelPerScope : MapSequence.fromMap(myScopePanels).values()) {
      res.add(panelPerScope.getCheckBox());
    }
    return res;
  }
  ...
  private JBPanel createPanelWithTestScope() {
    ...
    for (final JUnitRunType runType : MapSequence.fromMap(myScopePanels).keySet()) {
  ...
  private ComboBox<JUnitRunType> createRunTypeBox() {
    ComboBox<JUnitRunType> res;
    res = new ComboBox<JUnitRunType>(JUnitRunTypes.values());
  ...
  public InProcessJBCheckBox getCurrentInProcessCheckBox() {
    return MapSequence.fromMap(myScopePanels).get(myRunType).getCheckBox();
  }
  ...
  public void apply(final JUnitSettings_Configuration configuration) {
    ...
    for (JUnitRunType runType : JUnitRunTypes.values()) {
      PanelPerScope panelPerScope = MapSequence.fromMap(myScopePanels).get(runType);
      configuration.setInProcessFlagForRunType(ix, panelPerScope.getCheckBox().isSelected());
      ix++;
    }
  }
  ...
  public void reset(JUnitSettings_Configuration settings) {
    ...
    for (JUnitRunType runType : JUnitRunTypes.values()) {
      PanelPerScope panelPerScope = MapSequence.fromMap(myScopePanels).get(runType);
      panelPerScope.getCheckBox().setSelected(settings.getInProcessPropertyForRunType(ix));
      ix++;
    }
  ...
  public static final class PanelPerScope {
    ...
    private final InProcessJBCheckBox myCheckBox = createInProcessCheckBox();
    ...
    public InProcessJBCheckBox getCheckBox() {
      return myCheckBox;
    }
