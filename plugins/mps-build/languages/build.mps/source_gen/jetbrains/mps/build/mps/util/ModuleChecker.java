...
public final class ModuleChecker {
  ...
  private void generatorSourceLanguage(CheckType type) {
    ...
    if (type.doCheck && (SLinkOperations.getTarget(generator, LINKS.sourceLanguage$A51U) == null)) {
  ...
  private void collectDependencies(CheckType type, List<SNode> previous) {
    ...
    for (String path : myModuleDescriptor.getJavaLibs()) {
      ...
      if (path.endsWith(".jar")) {
        if (type.doCheck) {
          if (!(Sequence.fromIterable(SNodeOperations.ofConcept(BuildMps_Module__BehaviorDescriptor.getDependenciesUnwrapped_id3QtfwKhgryb.invoke(module), CONCEPTS.BuildMps_ModuleDependencyJar$Rm)).any(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
  ...
  private static SNode createBuildMps_ModuleDependencyOnModule_yr5c5g_a0a0a0a31a63(SNode node0) {
    SNodeBuilder rootBuilder1 = new SNodeBuilder().init(CONCEPTS.BuildMps_ModuleDependencyOnModule$1C);
    rootBuilder1.setReferenceTarget(LINKS.module$kGi0, node0);
    rootBuilder1.setProperty(PROPS.reexport$kN5t, PROPS.reexport$kN5t.getType().toString(false));
