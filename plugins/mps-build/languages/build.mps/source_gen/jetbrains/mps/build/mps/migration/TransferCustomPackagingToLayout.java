...
public class TransferCustomPackagingToLayout extends MigrationScriptBase {
      ...
      for (SNode ideaPlugin : CommandUtil.instances(CommandUtil.selectScope(null, context), CONCEPTS.BuildMps_IdeaPlugin$po, false)) {
        ...
        for (SNode content : ListSequence.fromList(SLinkOperations.getChildren(ideaPlugin, LINKS.content$9T6D))) {
          if (SNodeOperations.isInstanceOf(content, CONCEPTS.BuildMps_IdeaPluginModule$rY)) {
            if (!(SPropertyOperations.getBoolean(SNodeOperations.cast(content, CONCEPTS.BuildMps_IdeaPluginModule$rY), PROPS.customPackaging$31Ex))) {
              ListSequence.fromList(modulesToPackage).addElement(SNodeOperations.cast(content, CONCEPTS.BuildMps_IdeaPluginModule$rY));
            } else {
              autoPackaging = false;
            }
  ...
  private static SNode createBuildMpsLayout_ModuleJars_2skbva_a0a2a0a0(SNode node0) {
    SNodeBuilder rootBuilder1 = new SNodeBuilder().init(CONCEPTS.BuildMpsLayout_ModuleJars$MZ);
    rootBuilder1.setReferenceTarget(LINKS.module$iRYT, node0);
