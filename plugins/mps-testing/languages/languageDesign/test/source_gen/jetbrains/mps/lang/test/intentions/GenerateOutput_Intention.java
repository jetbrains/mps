...
public final class GenerateOutput_Intention extends AbstractIntentionDescriptor implements IntentionFactory {
    ...
    public void execute(final SNode node, final EditorContext editorContext) {
      ...
      if (ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.outputNodes$IIDt)).count() != CollectionSequence.fromCollection(output).count()) {
        ...
        for (SNode n : CollectionSequence.fromCollection(output)) {
          ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.outputNodes$IIDt)).addElement(createTestNode_8ua06z_a0a0a1a4a0(n));
        }
      } else {
        {
          Iterator<SNode> n_it = CollectionSequence.fromCollection(output).iterator();
          Iterator<SNode> current_it = ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.outputNodes$IIDt)).iterator();
          SNode n_var;
          SNode current_var;
          while (n_it.hasNext() && current_it.hasNext()) {
            n_var = n_it.next();
            current_var = current_it.next();
            SLinkOperations.setTarget(current_var, LINKS.nodeToCheck$OBcW, n_var);
          }
        }
      }
  ...
  private static MigrationScript check_hr1otd_a0a0a0a0c0e9(MigrationAspectDescriptor checkedDotOperand, SNode it) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getScript((int) IMigrationUnit__BehaviorDescriptor.fromVersion_id4uVwhQyFcnl.invoke(it));
    }
    return null;
  }
  private static MigrationAspectDescriptor check_hr1otd_a0a0a0a0a2a4j(LanguageRuntime checkedDotOperand) {
    if (null != checkedDotOperand) {
  ...
  private static MigrationScript check_hr1otd_a1a0a0a0a0i9(MigrationAspectDescriptor checkedDotOperand, SNode it) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getScript((int) IMigrationUnit__BehaviorDescriptor.fromVersion_id4uVwhQyFcnl.invoke(it));
    }
    return null;
  }
  private static MigrationAspectDescriptor check_hr1otd_a0b0a0a0a0a8j(LanguageRuntime checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getAspect(MigrationAspectDescriptor.class);
    }
    return null;
