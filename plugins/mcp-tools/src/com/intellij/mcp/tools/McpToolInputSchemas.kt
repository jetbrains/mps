...
private fun JsonObject.optionalStringAlias(fields: List<String>, path: String): String? {
  for (field in fields) {
    if (has(field)) {
      return optionalString(field, path)
    }
  }
  return null
}
...
private fun JsonObject.optionalInt(field: String, path: String): Int? {
  val element = optionalElement(field) ?: return null
  if (!element.isJsonPrimitive || !element.asJsonPrimitive.isNumber || !element.asString.matches(Regex("-?\\d+"))) {
...
private fun stringListOrString(element: JsonElement, path: String): List<String> {
  if (element.isJsonPrimitive && element.asJsonPrimitive.isString) {
    ...
    if (value.isBlank()) {
      throw ToolInputSchemaException("Missing '$path'")
    }
    return listOf(value)
  }
  if (!element.isJsonArray) {
  ...
  return element.asJsonArray.mapIndexed { index, item ->
    if (!item.isJsonPrimitive || !item.asJsonPrimitive.isString || item.asString.isBlank()) {
      throw ToolInputSchemaException("Missing '$path[$index]'")
    }
    item.asString
  }
}
