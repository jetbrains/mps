...
These `mps-*` skills and this guide were installed by the `mps_mcp_initialize_project_for_agents` MCP tool into the repository / workspace root — which may be an ancestor of the MPS project directory when the MPS project lives in a subdirectory (e.g. `tools/BigProject`). The `mps_mcp_*` tools themselves act on whichever MPS project is open in MPS; if a tool reports "no project" or "multiple projects opened", give it the MPS project's own base directory, not this root. If the skills or this guide become stale or incomplete, ask the user for approval to refresh: delete every `mps-*` skill folder (keeping any project-local `*-dsl` skills), re-run the tool, then carefully merge its returned `agentsFileText` back into this file while preserving the Project-Specific Notes below. See the tool's description for the full refresh contract.
