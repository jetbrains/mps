  ...
  /*package*/ SNode convertStatement(TryStatement x) throws JavaParseException {
    ...
    if (x.catchBlocks != null) {
      for (int i = 0, c = x.catchArguments.length; i < c; ++i) {
        ...
        SNode local = SLinkOperations.getTarget(SNodeOperations.cast(convertStatement(x.catchArguments[i]), CONCEPTS.LocalVariableDeclarationStatement$BI), LINKS.localVariableDeclaration$cjR0);
    ...
    if ((finallyBlock != null)) {
      ...
      for (int i = 0; i < catchBlocks.size(); i++) {
        ...
        SLinkOperations.setTarget(catchClause, LINKS.catchBody$T0$W, getStatementListFromStatement(catchBlock, x.catchBlocks[i]));
        SLinkOperations.setTarget(catchClause, LINKS.throwable$T0lV, lvd);
      }
      SLinkOperations.setTarget(tryStatement, LINKS.finallyBody$8W3M, getStatementListFromStatement(finallyBlock, x.finallyBlock));
      SLinkOperations.setTarget(tryStatement, LINKS.body$1aAh, getStatementListFromStatement(tryBlock, x.tryBlock));
      return tryStatement;
    } else {
      ...
      for (int i = 0; i < catchBlocks.size(); i++) {
        ...
        SNode lvd = ListSequence.fromList(catchArgs).getElement(i);
        SNode catchClause = SConceptOperations.createNewNode(MetaAdapterFactory.getConcept(0xf3061a5392264cc5L, 0xa443f952ceaf5816L, 0x10f39a56e2fL, "jetbrains.mps.baseLanguage.structure.CatchClause"));
        ListSequence.fromList(SLinkOperations.getChildren(tryCatchStatement, LINKS.catchClause$yGV3)).addElement(catchClause);
        SLinkOperations.setTarget(catchClause, LINKS.catchBody$T0$W, getStatementListFromStatement(catchBlock, x.catchBlocks[i]));
        SLinkOperations.setTarget(catchClause, LINKS.throwable$T0lV, lvd);
      }
      SLinkOperations.setTarget(tryCatchStatement, LINKS.body$I$eg, getStatementListFromStatement(tryBlock, x.tryBlock));
