...
public class ASTConverterWithExpressions extends ASTConverter {
  private final Logger LOG = Logger.getLogger(ASTConverterWithExpressions.class);
  ...
  /*package*/ SNode convertBinaryOperation(Expression left, Expression right, SNode binaryOperation) throws JavaParseException {
    SLinkOperations.setTarget(binaryOperation, LINKS.leftExpression$sEj, convertExpressionWrap(left));
    SLinkOperations.setTarget(binaryOperation, LINKS.rightExpression$nvX, convertExpressionWrap(right));
  ...
  private void adjustClassReference(SNode clsType, SNode source, SReferenceLink role) {
    ...
    source.setReference(role, sref.describeTarget());
  }
  ...
  public SNode convertExpressionWrap(Expression expression) throws JavaParseException {
    ...
    if (expression != null) {
      ...
      for (int parenthsCreated = 0; parenthsCreated < parenthesisCount; parenthsCreated++) {
        ...
        SLinkOperations.setTarget(parenthesizedExpression, LINKS.expression$TlhM, result);
  ...
  public SNode convertExpression(Expression x) throws JavaParseException {
    ...
    } else if (x instanceof ReferenceExpression) {
      return convertExpression((ReferenceExpression) x);
  ...
  /*package*/ SNode convertExpression(ArrayAllocationExpression x) throws JavaParseException {
    ...
    if (x.initializer != null) {
      ...
      for (SNode initializer : initializers) {
        ListSequence.fromList(SLinkOperations.getChildren(arrayCreator, LINKS.initValue$4K91)).addElement(initializer);
      }
      SLinkOperations.setTarget(arrayCreator, LINKS.componentType$sXdl, compType);
      ...
      SLinkOperations.setTarget(genericNewExpression, LINKS.creator$BsHW, arrayCreator);
      ...
      for (SNode dim : dims) {
        ...
        ListSequence.fromList(SLinkOperations.getChildren(arrayCreator, LINKS.dimensionExpression$DH6N)).addElement(dimensionExpression);
        if (dim != null) {
          SLinkOperations.setTarget(dimensionExpression, LINKS.expression$4ICz, dim);
        }
      }
      SNode deepestComponentType = compType;
      while (SNodeOperations.isInstanceOf(deepestComponentType, CONCEPTS.ArrayType$rh)) {
        deepestComponentType = SLinkOperations.getTarget(SNodeOperations.cast(deepestComponentType, CONCEPTS.ArrayType$rh), LINKS.componentType$F$Gi);
      }
      SLinkOperations.setTarget(arrayCreator, LINKS.componentType$SJjq, deepestComponentType);
      SNode genericNewExpression = SConceptOperations.createNewNode(MetaAdapterFactory.getConcept(0xf3061a5392264cc5L, 0xa443f952ceaf5816L, 0x10ab8473cc5L, "jetbrains.mps.baseLanguage.structure.GenericNewExpression"));
      SLinkOperations.setTarget(genericNewExpression, LINKS.creator$BsHW, arrayCreator);
      return genericNewExpression;
    }
  ...
  /*package*/ SNode convertExpression(ArrayInitializer x) throws JavaParseException {
    ...
    for (SNode initializer : initializers) {
      ListSequence.fromList(SLinkOperations.getChildren(arrayLiteral, LINKS.item$LHuY)).addElement(initializer);
    }
    return arrayLiteral;
  }
  /*package*/ SNode convertExpression(ArrayReference x) throws JavaParseException {
    ...
    SLinkOperations.setTarget(accessExpression, LINKS.array$tTQe, convertExpressionWrap(x.receiver));
    SLinkOperations.setTarget(accessExpression, LINKS.index$LbBP, convertExpressionWrap(x.position));
  ...
  /*package*/ SNode convertExpression(Assignment x) throws JavaParseException {
    if (x instanceof CompoundAssignment) {
      ...
      SLinkOperations.setTarget(assignmentExpression, LINKS.lValue$splI, convertExpressionWrap(x.lhs));
      SLinkOperations.setTarget(assignmentExpression, LINKS.rValue$spNK, convertExpressionWrap(x.expression));
  ...
  /*package*/ SNode convertExpression(CompoundAssignment x) throws JavaParseException {
    if (x instanceof PostfixExpression) {
      ...
      SLinkOperations.setTarget(op, LINKS.lValue$splI, convertExpressionWrap(x.lhs));
      SLinkOperations.setTarget(op, LINKS.rValue$spNK, convertExpressionWrap(x.expression));
      return op;
    }
  ...
  /*package*/ SNode convertExpression(PrefixExpression x) throws JavaParseException {
    ...
    SLinkOperations.setTarget(op, LINKS.expression$uRUg, convertExpressionWrap(x.lhs));
    return op;
  }
  /*package*/ SNode convertExpression(CastExpression x) throws JavaParseException {
    ...
    SLinkOperations.setTarget(result, LINKS.type$XD7M, convertTypeReference(x.type));
    return result;
  }
  ...
  /*package*/ SNode convertExpression(QualifiedThisReference x) throws JavaParseException {
    ...
    adjustClassReference(SNodeOperations.cast(type, CONCEPTS.ClassifierType$bL), thisRef, LINKS.classConcept$zzjZ);
    return thisRef;
  }
  /*package*/ SNode convertExpression(SingleNameReference x) {
    ...
    varRef.setReference(LINKS.variableDeclaration$N1XG, ResolveInfo.of(new String(x.token)));
    return varRef;
  }
  /*package*/ SNode convertExpression(QualifiedNameReference x) {
    ...
    SPropertyOperations.assign(unkName, PROPS.tokens$J1uk, sb.toString());
    return unkName;
  }

  private String getTokens(Expression receiver) {
    String[] tokens;

    if (receiver instanceof SingleNameReference) {
      tokens = new String[1];
      tokens[0] = new String(((SingleNameReference) receiver).token);

    } else if (receiver instanceof QualifiedNameReference) {
      QualifiedNameReference nameRef = ((QualifiedNameReference) receiver);
  ...
  /*package*/ SNode convertExpression(MessageSend x) throws JavaParseException {
    ...
    if (x.receiver instanceof ThisReference && ((ThisReference) x.receiver).isImplicitThis()) {
      ...
      lmc.setReference(LINKS.baseMethodDeclaration$pyYw, ResolveInfo.of(methodName));
      ...
      if (receiver instanceof NameReference) {
        ...
        SPropertyOperations.assign(unkDotCall, PROPS.tokens$J1uk, getTokens(receiver));
        ...
        smc.setReference(LINKS.baseMethodDeclaration$pyYw, ResolveInfo.of(methodName));

        call = smc;

      } else {
        // we can already know that it should be an InstanceMethodCall
        SNode instCall = SConceptOperations.createNewNode(MetaAdapterFactory.getConcept(0xf3061a5392264cc5L, 0xa443f952ceaf5816L, 0x15003fd0d0b36dc8L, "jetbrains.mps.baseLanguage.structure.UnknownInstanceMethodCall"));
        SLinkOperations.setTarget(instCall, LINKS.operand$FSAL, convertExpressionWrap(x.receiver));
        SPropertyOperations.assign(instCall, PROPS.callee$uWRA, methodName);
    ...
    if ((call != null)) {
      ...
      addTypeArgs(x.typeArguments, SLinkOperations.getChildren(call, LINKS.typeArgument$jaIN));
    }
    return SNodeOperations.as(call, CONCEPTS.Expression$mB);
  }
  ...
  /*package*/ SNode convertExpression(ClassLiteralAccess x) throws JavaParseException {
    ...
    if (SNodeOperations.isInstanceOf(argType, CONCEPTS.ClassifierType$bL)) {
      ...
      adjustClassReference(SNodeOperations.cast(argType, CONCEPTS.ClassifierType$bL), result, LINKS.classifier$7Ex9);
      return result;
  ...
  /*package*/ SNode convertExpression(UnaryExpression x) throws JavaParseException {
    ...
    switch (operator) {
      case OperatorIds.MINUS:
        ...
        SLinkOperations.setTarget(unaryMinus, LINKS.expression$uRUg, convertExpressionWrap(x.expression));
        return unaryMinus;
      case OperatorIds.NOT:
        ...
        SLinkOperations.setTarget(notExpression, LINKS.expression$sv_H, convertExpressionWrap(x.expression));
      ...
      case OperatorIds.TWIDDLE:
        ...
        SLinkOperations.setTarget(twiddle, LINKS.expression$$XK9, convertExpressionWrap(x.expression));
        return twiddle;
      default:
  ...
  /*package*/ SNode convertExpression(ConditionalExpression x) throws JavaParseException {
    ...
    SLinkOperations.setTarget(tOp, LINKS.condition$nwNI, ifTest);
    SLinkOperations.setTarget(tOp, LINKS.ifTrue$Tg0R, thenExpr);
    SLinkOperations.setTarget(tOp, LINKS.ifFalse$Wbma, elseExpr);
    return tOp;
  }
  ...
  /*package*/ SNode convertExpression(InstanceOfExpression x) throws JavaParseException {
    ...
    SLinkOperations.setTarget(instanceOfExpression, LINKS.leftExpression$StkV, expr);
    SLinkOperations.setTarget(instanceOfExpression, LINKS.classType$StzW, testType);
  ...
  /*package*/ SNode convertExpression(FieldReference x) throws JavaParseException {
    ...
    SLinkOperations.setTarget(result, LINKS.operand$w6IR, convertExpressionWrap(x.receiver));
    ...
    SLinkOperations.setTarget(result, LINKS.operation$gs9E, fieldRef);
    fieldRef.setReference(LINKS.fieldDeclaration$H7Ag, ResolveInfo.of(new String(x.token)));
    return result;
  }
  /*package*/ SNode convertExpression(QualifiedAllocationExpression x) throws JavaParseException {
    ...
    if (x.anonymousType != null) {
      ...
      create = anonCreate;
      SLinkOperations.setTarget(result, LINKS.creator$BsHW, anonCreate);
      SNode cls = SNodeOperations.cast(convertTypeDecl(x.anonymousType), CONCEPTS.AnonymousClass$Bt);
      SLinkOperations.setTarget(anonCreate, LINKS.cls$Saf6, cls);
  ...
  /*package*/ SNode convertExpression(ReferenceExpression x) throws JavaParseException {
    SNode unkMRef = SConceptOperations.createNewNode(MetaAdapterFactory.getConcept(0xacfc188dd5d64598L, 0xb3706f4a983f05b2L, 0x4b3463a3c4af5feeL, "jetbrains.mps.baseLanguage.methodReferences.structure.UnknownMethodReference"));

    // Either a type or an object
    if (x.lhs instanceof NameReference) {
      // handle this special case
      SPropertyOperations.assign(unkMRef, PROPS.tokens$J1uk, getTokens(x.lhs));
    } else {
      SPropertyOperations.assign(unkMRef, PROPS.targetResolved$e43v, true);
      SLinkOperations.setTarget(unkMRef, LINKS.target$Woec, convertExpressionWrap(x.lhs));
    }
    ...
    return SNodeOperations.as(unkMRef, CONCEPTS.Expression$mB);
  }
