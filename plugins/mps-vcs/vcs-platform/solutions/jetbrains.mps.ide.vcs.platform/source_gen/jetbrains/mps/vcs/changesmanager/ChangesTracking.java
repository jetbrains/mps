...
public class ChangesTracking {
  ...
  private FileStatus calcStatus() {
    ...
    if (ds instanceof FileSystemBasedDataSource) {
      List<FileStatus> statuses = ((FileSystemBasedDataSource) ds).getAffectedFiles()
                                                                  .stream()
                                                                  .filter(Objects::nonNull)
                                                                  .map(VirtualFileUtils::getProjectVirtualFile)
                                                                  .filter(Objects::nonNull)
                                                                  .map(fsm::getStatus)
                                                                  .distinct()
                                                                  .collect(Collectors.toList());
      if (statuses.stream().anyMatch(st -> st == FileStatus.MERGED_WITH_CONFLICTS || st == FileStatus.MERGED_WITH_BOTH_CONFLICTS)) {
