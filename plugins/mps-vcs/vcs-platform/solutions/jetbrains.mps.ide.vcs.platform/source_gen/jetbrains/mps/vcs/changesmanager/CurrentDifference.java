...
public class CurrentDifference {
  private static final Logger LOG = LogManager.getLogger(CurrentDifference.class);
  ...
  /*package*/ void setChangeSet(@NotNull ChangeSetImpl changeSetImpl) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("change set size " + ListSequence.fromList(changeSetImpl.getModelChanges()).count());
    }
    if (LOG.isDebugEnabled()) {
      LOG.debug("removing change set");
    }
    removeChangeSet();
    if (LOG.isDebugEnabled()) {
      LOG.debug("done removing change set");
    }
    if (LOG.isDebugEnabled()) {
      LOG.debug("change update started");
    }
    myBroadcaster.changeUpdateStarted();
    if (LOG.isDebugEnabled()) {
      LOG.debug("done change update started");
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("done changes added");
    }
    if (LOG.isDebugEnabled()) {
      LOG.debug("change update finished");
    }
    myBroadcaster.changeUpdateFinished();
    if (LOG.isDebugEnabled()) {
      LOG.debug("done change update finished");
    }
  }
