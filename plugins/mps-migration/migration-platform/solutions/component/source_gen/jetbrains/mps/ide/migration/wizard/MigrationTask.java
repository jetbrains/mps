...
public class MigrationTask {
  ...
  protected void doRun(ProgressMonitor monitor) throws MigrationError {
    ...
    if (checkAndIncStage(3)) {
      NotMigratedLibsError nmle = NotMigratedLibsError.prepare(mySession, monitor.subTask(5, SubProgressKind.REPLACING));
      if (nmle.hasErrors()) {
        if (myHaltOnNotMigratedLibs) {
          throw nmle;
        } else {
          logProblems("Some dependent modules are not migrated, ignoring...", nmle, new LogHandler(Logger.getLogger(MigrationTask.class)));
        }
      }
    }

    if (checkAndIncStage(4)) {
      PreCheckError preCheckError = PreCheckError.prepare(mySession, monitor.subTask(10, SubProgressKind.REPLACING));
      if (preCheckError.hasErrors()) {
        if (myHaltOnFailedPrecheck) {
          throw preCheckError;
        } else {
          logProblems("Migration pre-check has failed, ignoring...", preCheckError, new LogHandler(Logger.getLogger(MigrationTask.class)));
        }
      }
    }
  ...
  protected void logProblems(String title, MigrationError err, @NotNull IMessageHandler handler) {
    // XXX suspicious method, provided there's also error(), would be nice to streamline error handling in a reasonable way
    if ((title != null && title.length() > 0)) {
      handler.handle(Message.error(getClass(), title, null, null));
    }
    handler.handle(Message.error(getClass(), err.getMessage(), null, err));
    handler.handle(Message.error(getClass(), "Listing all the problems of " + err, null, null));
    for (IssueKindReportItem p : err.getProblems(new EmptyProgressIndicator())) {
      IssueKindReportItem.PathObject po = IssueKindReportItem.PATH_OBJECT.get(p);
      String problemMsg = String.format(" - %s  (reason object: %s)", p.getMessage(), po);
      handler.handle(Message.error(getClass(), problemMsg, po, null));
    }
  }
