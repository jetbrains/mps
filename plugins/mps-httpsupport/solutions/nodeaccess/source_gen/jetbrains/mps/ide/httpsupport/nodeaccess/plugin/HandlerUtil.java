...
public class HandlerUtil {
  ...
  public static Project getProjectForNode(Project project, SNodeReference nodeRef) {
    if (nodeRef == null) {
      return null;
    }
    SModelReference modelReference = nodeRef.getModelReference();
    ProjectManager projectManager = MPSCoreComponents.getInstance().getPlatform().findComponent(ProjectManager.class);
    if (modelReference == null || projectManager == null) {
      return null;
    }
    SModelId modelId = modelReference.getModelId();
    // Check first if the provided project contains the node
    if (project != null && ownsModel(project, modelId)) {
      return project;
    }
    // Scan projects for the node
    for (Project p : projectManager.getOpenedProjects()) {
      if (ownsModel(p, modelId)) {
        return p;
      }
    }
    return null;
  }
  private static boolean ownsModel(final Project p, final SModelId modelId) {
    return p.getModelAccess().computeReadAction(() -> {
      for (SModule module : p.getProjectModules()) {
        if (module.getModel(modelId) != null) {
          return true;
        }
      }
      return false;
  ...
  public static void showNoProjectIsAvailablePopup() {
    ApplicationManager.getApplication().invokeLater(() -> {
      JComponent component = (as_qa1yjq_a0a0a0a0a0a0lb(WindowManager.getInstance().findVisibleFrame(), IdeFrame.class)).getComponent();
  ...
  private static void showNotFoundPopup(final Project project, final String text) {
    final MPSProject mpsProject = as_qa1yjq_a0a0a34(project, MPSProject.class);
  ...
  private static <T> T as_qa1yjq_a0a0a0a0a0a0lb(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
  private static <T> T as_qa1yjq_a0a0a34(Object o, Class<T> type) {
    return (type.isInstance(o) ? (T) o : null);
  }
}
