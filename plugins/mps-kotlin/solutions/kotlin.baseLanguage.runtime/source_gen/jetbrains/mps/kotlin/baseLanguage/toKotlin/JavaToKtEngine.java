...
public class JavaToKtEngine extends TypeConverterEngineWithClass<SNode, SNode> {
  public JavaToKtEngine() {
    ...
    this.declareMapping(CONCEPTS.ClassifierType$bL, new AnyClassConverter());
  }

  public static class AnyClassConverter implements TypeConverter<SNode, SNode> {
    @Override
    public SNode convert(SNode source, final TypeConverterEngine<SNode, SNode> engine) {
      ...
      SLinkOperations.setTarget(node, LINKS.javaClass$CQOW, SLinkOperations.getTarget(source, LINKS.classifier$cxMr));
      SLinkOperations.getChildren(node, LINKS.typeProjections$vhti).addAll(Sequence.fromIterable(IClassifierType__BehaviorDescriptor.getTypeParameters_id6r77ob2URYe.invoke(source)).select(new ISelector<SNode, SNode>() {
        public SNode select(SNode it) {
          return convertProjection(it, engine);
        }
      }).toListSequence());
      return node;
    }
