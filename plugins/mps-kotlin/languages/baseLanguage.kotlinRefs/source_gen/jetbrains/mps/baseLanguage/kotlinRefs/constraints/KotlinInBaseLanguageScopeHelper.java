...
public class KotlinInBaseLanguageScopeHelper {
  public static Scope getScopeForAccessor(SNode referenceNode, SNode contextNode, final PropertyAccessorKind kind) {
    return getScopeForMemeber(referenceNode, contextNode, CONCEPTS.VariableDeclaration$DF, PropertyAccessorSignature.class, (SourcedSignature signature) -> {
      // Only one kind of signature
      return ((PropertyAccessorSignature) signature.getSignature()).getKind() == kind;
    });
  }

  public static Scope getScopeForMemeber(SNode referenceNode, SNode contextNode, SAbstractConcept filteringConcept, Class<? extends MemberSignature> signatureKind, _FunctionTypes._return_P1_E0<? extends Boolean, ? super SourcedSignature> signatureFilter) {
    ...
    SRepository repository = contextNode.getModel().getRepository();
    ClassMemberVisitor visitor = new ClassMemberVisitor(signatureKind, signatureFilter, repository);
