...
public class SignatureScopeHelper {
   ...
   */
  public static Iterable<SignatureScope> getFunctionScopeParts(final CallReceiver receiver, SNode contextNode, SignatureFilter<FunctionSignature> filter) {
    if (receiver == null) {
      return null;
    }

    // Compute type in isolation, otherwise type may be null
    SNode type = TypecheckingFacade.getFromContext().computeIsolated(() -> receiver.getTypeReference().compute());

    if (receiver.isStaticReceiver()) {
      // TODO two separate sets?
      return Sequence.<SignatureScope>singleton(CompositeSignatureScope.of(IType__BehaviorDescriptor.getCompanionInstanceScope_id1ODRHGtugRP.invoke(type, filter), IType__BehaviorDescriptor.getStaticScope_id1ODRHGtufGw.invoke(type, filter)));
    } else {
  ...
  public static Iterable<SignatureScope> getFunctionScopeParts(CallReceiver receiver, SNode contextNode) {
    ...
    if ((context != null)) {
      ...
      if (isTargetFromLink || isTargetFromNode) {
        ...
        if (SNodeOperations.isInstanceOf(SLinkOperations.getTarget(context, LINKS.operand$YS5t), CONCEPTS.ReceiverType$$f)) {
          return MultiTuple.<SNode,Boolean>from(SLinkOperations.getTarget(SNodeOperations.cast(SLinkOperations.getTarget(context, LINKS.operand$YS5t), CONCEPTS.ReceiverType$$f), LINKS.type$NVFj), true);
        }

        SNode type = TypecheckingFacade.getFromContext().computeIsolated(() -> SNodeOperations.as(TypecheckingFacade.getFromContext().getTypeOf(SLinkOperations.getTarget(context, LINKS.operand$YS5t)), CONCEPTS.IType$Ni));
