...
public class FlexibleBlock_UnforceMultiLine {
  ...
  /*package*/ static AbstractCellAction createAction_BACKSPACE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        SPropertyOperations.assign(node, PROPS.forceMultiLine$d9kx, false);
        if (ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.statements$R3pt)).all((it) -> SNodeOperations.isInstanceOf(it, CONCEPTS.EmptyStatement$cT))) {
          ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.statements$R3pt)).clear();
          SelectionUtil.selectLabelCellAnSetCaret(editorContext, node, "emptyCell", 0);
        } else {
          editorContext.selectWRTFocusPolicy(ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.statements$R3pt)).first());
        }
      }
      @Override
      ...
      public boolean canExecute_internal(EditorContext editorContext, SNode node) {
        // Case with empty statements might not make sense
        return SPropertyOperations.getBoolean(node, PROPS.forceMultiLine$d9kx) && (ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.statements$R3pt)).all((it) -> SNodeOperations.isInstanceOf(it, CONCEPTS.EmptyStatement$cT)) || (IStatementHolder__BehaviorDescriptor.asSingleExpression_id18X2O0FvKfA.invoke(node) != null));
      }
