...
>
> **When in doubt, ask which project.** When several MPS projects are open under one repository / VCS root, the `mps_mcp_*` tools accept *any* valid project path and silently act on exactly that project — they do **not** warn that you may have meant a different open project, and because all open projects share one module repository, querying or editing the wrong one returns the wrong nodes (or gets a cross-project write refused). If the intended project is not obvious from the user's request or the current editor focus (`mps_mcp_get_current_editor_root_node`), do **not** guess — show the `mps_mcp_list_open_projects` entries and ask the user which project the query or change should run against.
